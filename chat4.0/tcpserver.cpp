...
{
    ui->setupUi(this);
    setFixedSize(700,360);//把对话框大小固定为350*180
...
void TcpServer::initServer()//对变量进行了初始化，然后设置了按钮的状态。
{
    payloadSize = 64*1024;
    TotalBytes = 0;
    bytesWritten = 0;
    bytesToWrite = 0;
    ui->serverStatusLabel->setText(tr("请选择需要传送的文件"));
...
{
    tcpServer->close();
    ui->serverStatusLabel->setText("对方绝接收！！！");
}
