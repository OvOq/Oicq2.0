...
void TcpClient::readMessage()
{
    ...
    {
        if((tcpClient->bytesAvailable()>=sizeof(qint)*2)&&(fileNameSize==0))
        {
            n>>TotalBytes>>fileNameSize;
            bytesReceived+=fileNameSize;
        }
        if((tcpClient->bytesAvailable()>=fileNamwSize)&&(fileNameSize!=0))
        {
            in>>fileName;
            bytesReceived+=fileNameSize;
    ...
    ui->tcpClientStatusLabel->setText(tr("已接受%1MB(%2MB/s)"
                       "\n共%3MB已用时:%4秒\n估计剩余时间:%5秒")
                        .arg(bytesReceived/(1024*1024))
                        .arg(speed *1000.(1024*1024),0,,2)
                        .arg(TotalBytes/(1024*1024))
                        .arg(useTime/1000,,0)
                        .arg(TotalBytes/speed/1000 - useTime/1000,0,,0));
    if(bytesReceived==TotalBytes)
    {
        localFile->close();
        tcpClient->close();
...
void TcpClient::displayError(QAbstractSocket::SocketError socketError)
{
    switch(socketError)
    {
        case QAbstractSocket::RemoteHostError:break;
